...
class TestActiveInjector:
    ...
    test2 = (
        ...
        "from duecredit.tests.mod import TestClass1; c = TestClass1()",
        "c.testmeth1",
    )
    test3 = (
        "TestClass12.Embed.testmeth1",
        "from duecredit.tests.mod import TestClass12; c = TestClass12.Embed()",
        "c.testmeth1",
    )
...
def test_find_iobject() -> None:
    ...
    assert find_object(mod, "TestClass1.testmeth1") == (
        mod.TestClass1,
        "testmeth1",
        mod.TestClass1.testmeth1,
    )
    assert find_object(mod, "TestClass12.Embed.testmeth1") == (
        mod.TestClass12.Embed,
        "testmeth1",
        mod.TestClass12.Embed.testmeth1,
    )
...
def test_no_double_activation() -> None:
    ...
    try:
        ...
        injector.activate()
        assert (
            __builtin__.__import__ is duecredited__import__
        )  # we didn't decorate again
    finally:
        injector.deactivate()
...
def test_get_modules_for_injection() -> None:
    ...
    assert get_modules_for_injection() == [
        "mod_biosig",
        "mod_dipy",
        "mod_matplotlib",
        "mod_mdp",
...
def test_cover_our_injections() -> None:
    ...
    for modname in get_modules_for_injection():
        mod = __import__(
            "duecredit.injections." + modname, fromlist=["duecredit.injections"]
        )
        mod.inject(inj)
